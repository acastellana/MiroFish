      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" title="Refresh图谱">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">Graph DataLoading...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">生成完成后将自动Start Build图谱</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">Graph Construction中</p>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  上传文档后，LLM分析文档内容，自动生成适合舆论模拟的本体结构（Entity Types + Relationship Types）
                </div>
              </div>
              
              <!-- 本体生成Progress -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">生成Progress</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的Entity Types ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的Relationship Types ({{ projectData.ontology.relation_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">构建Progress</div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">关系Edges</span>
                  </div>
                  <div class="result-item">
...
const ontologyProgress = ref(null) // 本体生成Progress
const currentPhase = ref(-1) // -1: 上传中, 0: 本体Generating, 1: Graph Construction, 2: 完成
const selectedItem = ref(null) // 选中的Nodes或Edges
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Graph Construction中'
  if (currentPhase.value === 0) return '本体Generating'
  return '初始化中'
})
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'None待上传的文件，请Back首页重新Actions'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: '正在Upload File并分析文档...' }
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: '正在启动Graph Construction...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Graph Construction任务已启动...'
      
      // 保存 task_id 用于轮询
      const taskId = response.data.task_id
      
      // 启动Graph Data轮询（独立于任务Status轮询）
      startGraphPolling()
      
      // 启动任务Status轮询
      startPollingTask(taskId)
    } else {
      error.value = response.error || '启动Graph ConstructionFailed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '启动Graph ConstructionFailed: ' + (err.message || '未知Error')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph Construction完成，正在加载完整数据...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // 更新Progress显示为完成Status
        buildProgress.value = {
          progress: 100,
          message: '构建完成，正在加载图谱...'
        }
