    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context, auto-invokes tools to organize entities and relations from the knowledge graph, initializes agents, and assigns unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow rate, recommendation algorithms, agent active periods, posting frequency, event triggers, and more based on Simulation Requirement and reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation round config - only shown after config is generated and rounds calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Settings</span>
                <span class="section-desc">MiroFish auto-plans a simulation of <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours; each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> min of elapsed time</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>Est. time for 100 agents: ~{{ Math.round(customMaxRounds * 0.6) }} min</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        Est. time for 100 agents: {{ Math.round(autoGeneratedRounds * 0.6) }} min
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first-time runs, strongly recommend switching to 'Custom mode' to reduce round count for faster preview and lower error risk 27a1</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Experience summary and behavior style preference</span>
              </div>
              <div class="dimension-card">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Agent connections and interaction graph</span>
              </div>
            </div>
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // return null if config not generated yet
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // return null if config data incomplete
  }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User chose to keep auto-generated rounds, don't pass max_rounds
    addLog(`Start Simulation, using auto-configured rounds: ${autoGeneratedRounds.value} rounds`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Existing completed setup detected, reusing it')
      ...
      addLog('Starting setup progress polling...')
      // Start polling progress
      startPolling()
      // Start real-time fetching of profiles
      startProfilesPolling()
    } else {
      addLog(`Setup failed: ${res.error || 'Unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Parse phase info and output detailed logs
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog('✓ Setup work completed')
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(`✗ Setup failed: ${data.error || 'Unknown error'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
