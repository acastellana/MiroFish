    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing — consider refreshing the graph shortly</span>
...
const renderGraph = () => {
  ...
  // Track how many edges between each node pair have been processed
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread out multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      ...
      // Flip curvature if edge direction is opposite to normalized direction
      // Ensures all edges are distributed in the same reference frame without overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to pull isolated node clusters toward the center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw arc starting and ending at the node
      const loopRadius = 30
      // Start from right side of node, loop back around
    ...
    // Compute curve control point - dynamically adjusted by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset proportional to distance, ensures curve is clearly visible
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Only let simulation cool down if a real drag occurred
        if (d._isDragging) {
  ...
  // Click on blank area to close details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
