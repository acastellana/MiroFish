            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content done, but full section may not be complete) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button in the same viewport position
...
const toolConfig = {
  ...
  'panorama_search': {
    ...
    color: 'blue',
    icon: 'globe' // globe icon - represents global search
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // users icon - represents interviews
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const edgesMatch = text.match(/总Edges数:\s*(\d+)/)
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Parse selection rationale per person
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format 1: N. **Name (index=X)**: reason
        ...
        // e.g. - Selected Parent_601 (index 0): as a representative of parents...
        if (!headerMatch) {
        ...
        // e.g. - **Parent_601 (index 0)**: as a representative of parents...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation of rationale (exclude trailing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (nameRoleMatch) {
        ...
        // Set selection rationale for this person
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question is preceded by "1.", handle specially
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compat with old format: single platform marker)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer — copy as default only if not placeholder text
          ...
          // No platform marker (very old format), use whole response as answer
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Try to extract edges info (if present)
    const edgesSection = text.match(/### 相关Edges:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
    ...
    // Try to extract nodes info (if present)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Active Memory (${props.result.facts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No active memory'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if this is a platform placeholder text
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering found or only one, return full answer
      if (matches.length <= 1) {
    ...
    // Check if a question has dual-platform answers (filtered for placeholders)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 (## xxx) since section title is shown in outer wrapper
  ...
  // Clean <br> before paragraph open tag following block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists: keep incrementing when <ol> items are separated by paragraphs
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task completed, scroll to top; otherwise scroll to bottom to follow latest log
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "最终答案:" (Chinese final answer marker)
