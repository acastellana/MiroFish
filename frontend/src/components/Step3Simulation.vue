...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid leftover state from previous run
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Extra check: backend may not have updated runner_status yet but platform reports done
      // Detect via twitter_completed and reddit_completed flags
...
const checkPlatformsCompleted = (data) => {
  ...
  // Use actions_count to determine if platform is enabled
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // No auto-scroll, let user browse the timeline freely
