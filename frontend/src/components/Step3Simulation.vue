...
const phase = ref(0) // 0: 未开始, 1: Running, 2: Completed
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 已清理旧的模拟日志，重新Start Simulation')
      }
      addLog('✓ 模拟引擎启动Success')
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`停止Failed: ${res.error || '未知Error'}`)
    }
  } catch (err) {
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 检测模拟是否Completed（通过 runner_status 或平台完成Status判断）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 额外检查：如果后端还没来得及更新 runner_status，但平台已经报告完成
      // 通过检测 twitter_completed 和 reddit_completed Status判断
    ...
    console.warn('获取运行StatusFailed:', err)
  }
}

// 检查所有启用的平台是否Completed
const checkPlatformsCompleted = (data) => {
  // 如果None任何平台数据，Back false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('获取详细StatusFailed:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report Generation请求已Send，请稍候...')
    return
  }
