...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards per row to ensure each row is centered
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (always record latest regardless of animation)
        ...
        // Use debounce delay for state switching to prevent quick flickering
        // Expand delay is shorter (50ms), collapse delay longer (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to execute (may have been overridden)
          ...
          setTimeout(() => {
            ...
            // After animation, check if there is a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before executing to avoid too-fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink viewport bottom up, requires more scroll to trigger expand
...
/* Use CSS background pattern to create evenly spaced square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Positioned from top-left; height changes only extend at bottom, not affecting existing grid */
